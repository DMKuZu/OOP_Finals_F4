...
public class Current_Enemy {
    ...
    protected List<String> SKILL_NAME;
    protected List<Integer> BASE_DMG;

    protected String enemyType;

    protected Chosen_Hero BOSS;


    protected final Skill_Calculator calc = new Skill_Calculator();
    ...
    public Current_Enemy(Chosen_Hero hero,int worldCtr, int encounterCtr, boolean isNorm){
        ...
        switch(hero.getNAME().toLowerCase()){
            case "radea":
                switch (worldCtr) {
                    case 1:
                        if(encounterCtr == 8) BOSS = new Mazupe(STATS,CURRSTATS,this);
                        else new Quaceron_Enemy(encounterCtr,isNorm).init(this);
                        break;
                    case 2:
                        if(encounterCtr == 8) BOSS = new Zipau(STATS,CURRSTATS,this);
                        else new Azrueian_Enemy(encounterCtr,isNorm).init(this);
                        break;
                    case 3:
                        if(encounterCtr == 8) BOSS = new Radea(STATS,CURRSTATS,this);
